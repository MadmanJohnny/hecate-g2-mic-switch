...
托盘小图标继续在后台工作，语音输入控制不受影响。

**鼠标悬停**在托盘图标上可以看到当前状态（未启动 / 待命 / 录音中）；
**左键单击**重新打开主窗口；**右键**弹出菜单（显示主界面 / 开始·停止桥接 / 退出）。
要真正退出就用托盘菜单里的「退出」；不想用这个行为，可以在「设置 → 行为」里
取消勾选「点击关闭按钮时最小化到托盘」，之后点 ✕ 就会直接退出。
...
Tk 默认不设 `WM_GETICON`，系统会落到窗口类自带的那根羽毛（Tk 的 logo）上，
所以程序在窗口映射完成后用 `WM_SETICON` 把图标换成自己的。

**Q：为什么不用 pystray / Pillow 做托盘？**
为了保持**运行时零第三方依赖**——托盘用 `Shell_NotifyIcon`，
图标按 ICO 格式在内存里生成，纯 ctypes 即可。

...
**点关闭按钮不会退出程序。** 点右上角的 ✕ 之后，程序会收进任务栏右下角的
托盘小图标继续在后台工作，语音输入控制不受影响。托盘图标：
...
鼠标悬停显示当前状态；**左键单击**重新打开主窗口；**右键**弹出菜单
（显示主界面 / 开始·停止桥接 / 退出）。要真正退出就用托盘右键菜单里的
「退出」；不想用这个行为，可以在「设置 → 行为」里取消勾选
「点击关闭按钮时最小化到托盘」，之后点 ✕ 就会直接退出。

<p align="center">
  <img src="docs/images/icon-preview.png" alt="图标各尺寸" width="430">
</p>
...
**Q：点了关闭按钮，程序去哪了？**
默认不退出，而是收进任务栏右下角的托盘（就是那个圆形耳机小图标）。
左键单击可以重新打开窗口，右键菜单里有「退出」。
不想这样就在「设置 → 行为」里取消勾选「点击关闭按钮时最小化到托盘」，
之后点 ✕ 会像以前一样直接退出。

**Q：托盘图标为什么是自己画的？**
为了继续保持**运行时零第三方依赖**——托盘用 `Shell_NotifyIcon`，
图标按 ICO 格式在内存里算出来（`src/tray_icon.py`），
所以不需要 pystray / Pillow / pywin32。`assets/app.ico`（exe 图标）
就是同一份代码生成的。
...
│   ├── tray_icon.py            系统托盘图标（纯 ctypes，零依赖）
...
│   └── images/
├── assets/
│   └── app.ico                 exe 图标（由 src/tray_icon.py 生成）
